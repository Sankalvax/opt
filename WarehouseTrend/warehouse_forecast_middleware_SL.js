 ...
 * * Middleware for fetching Warehouse Product Rolling Forecast data.
 */
define(['N/https', 'N/log'], (https, log) => {

    // Warehouse forecast API endpoint
    const API_URL = 'https://platform.sankalvax.ai/api/warehouse-forecast';
    ...
    function onRequest(context) {
        ...
        try {
            log.debug('Starting Warehouse Forecast API Call', { url: API_URL });
            ...
            log.error('Warehouse Forecast Middleware Exception', { error: e.toString() });
            response.code = 500;
            response.write(JSON.stringify({
                success: false,
                error: 'An internal error occurred in the NetSuite warehouse forecast middleware.',
                details: e.toString()
            }));
        }
    }
